...
module.exports.csdl2openapi = function (
  ...
  function schemasForStructuredType(schemas, qualifier, name, type, suffix) {
    ...
    Object.keys(properties).forEach((name) => {
      ...
      if (property.$Kind == "NavigationProperty") {
        ...
        if (
          ...
          if (suffix === "-create")
            schemaProperties[name] = schema(property, "-create");
          if (suffix === "-update" && deepUpdate)
            schemaProperties[name] = schema(property, "-create");
        }
      } else {
        if (
          ...
          if (
            ...
            schemaProperties[name] = schema(property, "-update");
        }
      }
    });
    ...
    schemas[schemaName] = {
      title: title + (suffix.length > 0 ? ` (for ${suffix.substring(1)})` : ""),
      type: "object",
    };
    ...
    if (suffix === "-create" && required.length > 0)
