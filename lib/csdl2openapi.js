...
//TODO
// - ActionImports and FunctionImports in ER diagram
    ...
    function resourceDiagram(csdl) {
        ...
        Object.keys(entityContainer).filter(name => isIdentifier(name)).forEach(name => {
            ...
            if (resource.$Type) {
                diagram += comma
                    + '[' + name + '{bg:dodgerblue}]'
                    + '++-'
                    + (resource.$Collection ? '*' : (resource.$Nullable ? '0..1' : '1'))
                ...
                if (resource.$Action) {
                    ...
                    if (overload.$ReturnType) {
                        ...
                        if (type) {
                            diagram += '-'
                                + (overload.$ReturnType.$Collection ? '*' : (overload.$ReturnType.$Nullable ? '0..1' : '1'))
                                + '>[' + nameParts(overload.$ReturnType.$Type).name + ']';
                        }
                    }
                } else {
                    // TODO: extract return types, array to them
        ...
        if (diagram != '') {
            diagram = '\n\n## Entity Data Model\n![ER Diagram](https://yuml.me/diagram/class/'
                + diagram
                + ')\n\n### Legend\n![Legend](https://yuml.me/diagram/plain;dir:TB;scale:60/class/[External.Type{bg:whitesmoke}],[ComplexType],[EntityType{bg:orange}],[EntitySet_or_Singleton{bg:dodgerblue}])';
        }
    ...
    function tags(container) {
        ...
        Object.keys(container).filter(name => isIdentifier(name) && container[name].$Type).forEach(child => {
            ...
            const description = container[child][voc.Core.Description]
                || modelElement(container[child].$Type)[voc.Core.Description];
    ...
    function operationCreate(pathItem, element, name, sourceName, targetName, target, level, restrictions) {
        ...
        if (insertRestrictions.Insertable !== false) {
            pathItem.post = {
                ...
                tags: [sourceName],
                requestBody: {
                    description: modelElement(element.$Type)[voc.Core.Description] || 'New entity',
                    required: true,
                    content: {
                        'application/json': {
    ...
    function operationUpdate(pathItem, element, name, sourceName, targetName, target, level, restrictions) {
        ...
        if (updateRestrictions.Updatable !== false) {
            pathItem.patch = {
                ...
                tags: [sourceName],
                requestBody: {
                    description: modelElement(element.$Type)[voc.Core.Description] || 'New property values',
                    required: true,
                    content: {
                        'application/json': {
        ...
        const pathItem = {
            get: {
                ...
                parameters: prefixParameters.concat(parameters.map(p => {
                    ...
                    let description = [p[voc.Core.Description], p[voc.Core.LongDescription]].filter(t => t).join('  \n');
