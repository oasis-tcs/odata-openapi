...
module.exports.csdl2openapi = function (
    ...
    function vocabularies(alias) {
        const terms = {
            Capabilities: ['BatchSupported', 'CountRestrictions', 'DeleteRestrictions', 'FilterRestrictions',
                'IndexableByKey', 'InsertRestrictions', 'NavigationRestrictions',
    ...
    function operationRead(pathItem, element, name, sourceName, targetName, target, level, restrictions, byKey) {
        ...
        const navReadable = restrictions.ReadRestrictions && restrictions.ReadRestrictions.Readable;
           // || (byKey == true && restrictions.ReadByKeyRestrictions && restrictions.ReadByKeyRestrictions.Readable == true);

        if (navReadable || (navReadable === undefined && targetReadable)) {
            //TODO: summary and description on navigation segments from restrictions, only level==0 from targetRestrictions
            const summary = byKey || (level > 0 && !element.$Collection)
                ? targetRestrictions.ReadByKeyRestrictions && targetRestrictions.ReadByKeyRestrictions.Description
                : targetRestrictions.Description;
            const description = byKey || (level > 0 && !element.$Collection)
                ? targetRestrictions.ReadByKeyRestrictions && targetRestrictions.ReadByKeyRestrictions.LongDescription
                : targetRestrictions.LongDescription;
