...
module.exports.csdl2openapi = function (
    ...
    function pathItems(paths, prefix, prefixParameters, element, root, sourceTag, targetTag, level) {
        ...
        if (element.$Collection) {
            pathItem.post = operationCreate(element, sourceTag, targetTag, level);
     ...
     * @param {integer} level Number of navigation segments so far
     */
    function operationCreate(element, sourceTag, targetTag, level) {
        const operation = {
            summary: 'Add new entity to ' + (level > 0 ? 'related ' : '') + targetTag,
            tags: [sourceTag],
            requestBody: {
            ...
            responses: response(201, element.$Type, 'Created entity')
        };
        if (sourceTag != targetTag) operation.tags.push(targetTag);
        return operation;
    }
        ...
        return {
            ...
            requestBody: {
                ...
                content: {
                    'application/json': {
                        schema: {
                            $ref: '#/components/schemas/' + element.$Type + '-update'
                        }
                    }
                }
            },
            responses: response(204, null, 'Success')
        };
    }
    ...
    */
     ...
     */
    function response(code, typename, description) {
        const r = {};
        r[code] = {
