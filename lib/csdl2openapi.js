...
module.exports.csdl2openapi = function (
  ...
  function pathItemsWithNavigation(
    ...
    if (!type || level >= maxLevels) return;
      ...
      post: {
        ...
        responses: overload.$ReturnType
          ? response(
              200,
              "Success",
              stubIfNotKept(overload.$ReturnType),
  ...
  function pathItemFunction(
    ...
    for (const p of parameters) {
      ...
      if (
        ...
        param.in = "query";
        if (implicitAliases && SYSTEM_QUERY_OPTIONS.includes(p.$Name)) {
          param.name = "@" + p.$Name;
    ...
    if (code != 204) {
      ...
      if (type.$Collection) {
        s = {
          ...
          properties:
            csdl.$Version === "2.0"
              ? { __count: ref("count"), results: s } //TODO: "results" or "d"? "d" only for function imports?
              : {
        ...
        //TODO: OData V2 function imports return a "d" wrapper and their name instead of "value"
        s = { type: "object", properties: { value: s } };
      }
      if (csdl.$Version === "2.0") {
        s = {
          type: "object",
  ...
  function schemasForStructuredType(schemas, qualifier, name, type, suffix) {
    ...
    for (let [name, property] of model.propertiesOfStructuredType(type)) {
      if (
        ...
        property = stubIfNotKept(property);
      }
