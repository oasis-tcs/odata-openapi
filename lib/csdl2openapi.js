...
module.exports.csdl2openapi = function (
    ...
    const boundOverloads = overloadsByBindingType(csdl);
    ...
    function overloadsByBindingType(csdl) {
        const map = {};
        ...
        Object.keys(csdl).filter(name => isIdentifier(name)).forEach(namespace => {
            ...
            Object.keys(schema).filter(name => isIdentifier(name) && Array.isArray(schema[name])).forEach(operation => {
                schema[operation].filter(overload => overload.$IsBound).forEach(overload => {
                    const type = overload.$Parameter[0].$Type + (overload.$Parameter[0].$Collection ? '-c' : '');
                    if (!map[type]) map[type] = [];
                    //TODO: also record name
                    map[type].push({ name: qualifier + '.' + operation, overload: overload });
                });
            });
        });
        console.log(map);
    ...
    function pathItems(paths, prefix, prefixParameters, element, root, sourceTag, targetTag, level) {
        ...
        if (element.$Collection && (level < 2 || element.$ContainsTarget)) {
            ...
            pathItemsForBoundOperations(paths, prefix, prefixParameters, element, sourceTag);
        }
    ...
    function pathItemsWithKey(paths, prefix, prefixParameters, element, root, sourceTag, targetTag, level) {
        ...
        pathItemsForBoundOperations(paths, path, parameters, element, sourceTag, true);
     ...
     * @param {boolean} byKey read by key
     */
    function pathItemsForBoundOperations(paths, prefix, prefixParameters, element, sourceTag, byKey = false) {
        const overloads = boundOverloads[element.$Type + (!byKey && element.$Collection ? '-c' : '')] || [];
        overloads.forEach(item => {
            if (item.overload.$Kind == 'Action')
                pathItemAction(paths, prefix + '/' + item.name, prefixParameters, item.name, item.overload, sourceTag);
            else
                pathItemFunction(paths, prefix + '/' + item.name, prefixParameters, item.name, item.overload, sourceTag);
        });
    }
    ...
    */
     ...
     * @param {string} sourceTag Tag of path source
     */
    function pathItemAction(paths, prefix, prefixParameters, actionName, overload, sourceTag) {
        var parameters = overload.$Parameter;
        if (overload.$IsBound) parameters = parameters.slice(1);
        const pathItem = {
            post: {
                summary: 'Invoke action ' + nameParts(actionName).name,
                tags: [sourceTag || 'Service Operations'],
        ...
        if (pathItem.post.parameters.length == 0) delete pathItem.post.parameters;
        paths[prefix] = pathItem;
    }
    ...
    * @param {object} child Function import object
     ...
     * @param {string} sourceTag Tag of path source
     */
    function pathItemFunction(paths, prefix, prefixParameters, functionName, overload, sourceTag) {
        var parameters = overload.$Parameter;
        if (overload.$IsBound) parameters = parameters.slice(1);
        ...
        const pathItem = {
            get: {
                summary: 'Invoke function ' + nameParts(functionName).name,
                tags: [sourceTag || 'Service Operations'],
        ...
        if (pathItem.get.parameters.length == 0) delete pathItem.get.parameters;
