...
module.exports.csdl2openapi = function (
    ...
    const alias = {};
    const namespace = {};
    preProcess(csdl, boundOverloads, derivedTypes, alias, namespace);
     ...
     */
    function preProcess(csdl, boundOverloads, derivedTypes, alias, namespace) {
    ...
    function vocabularies(csdl, alias) {
        const terms = {
            ...
            Core: ['Computed', 'Description', 'Immutable', 'LongDescription', 'Permissions']
        };
        const v = {};
    ...
    function pathItems(paths, prefix, prefixParameters, element, root, sourceName, targetName, target, level) {
        ...
        if (element.$Collection && (level < 2 || element.$ContainsTarget)) {
            if (target) operationCreate(pathItem, element, name, sourceName, targetName, target, level);
    ...
    */
    ...
    function ref(typename, suffix = '') {
        ...
        if (typename.indexOf('.') != -1) {
            let np = nameParts(typename);
            name = namespace[np.qualifier] + '.' + np.name;
        }
        return {
            $ref: '#/components/schemas/' + name + suffix
        };
    }
        ...
        switch (element.$Type) {
            ...
            case 'Edm.Decimal':
                ...
                if (element.$Precision < 16) {
                    //TODO: consider $Scale
                    let limit = 10 ** element.$Precision;
                    s.maximum = limit - 1;
                    s.minimum = -s.maximum;
                }
                break;
