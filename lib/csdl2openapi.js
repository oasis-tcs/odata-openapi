...
module.exports.csdl2openapi = function (
        ...
        Object.keys(csdl).filter(name => isIdentifier(name)).forEach(name => {
            ...
            Object.keys(schema.$Annotations || {}).forEach(target => {
                ...
                if (!element) {
                    ...
                    //- if second segment contains (, it references a single overload: find it
                    //- otherwise loop over all overloads
    ...
    function resourceDiagram(csdl) {
        ...
        Object.keys(entityContainer).filter(name => isIdentifier(name) && entityContainer[name].$Type).forEach(name => {
    ...
    function operationCreate(pathItem, element, name, sourceName, targetName, target, level, restrictions) {
        ...
        if (insertRestrictions.Insertable !== false) {
            ...
            if (insertRestrictions.LongDescription) pathItem.patch.description = insertRestrictions.LongDescription;
    ...
    function operationDelete(pathItem, element, name, sourceName, target, level, restrictions) {
        ...
        if (deleteRestrictions.Deletable !== false) {
            ...
            if (deleteRestrictions.LongDescription) pathItem.delete.description = updateRestrictions.LongDescription;
        }
    }
    ...
    function pathItemsWithNavigation(paths, prefix, prefixParameters, type, root, sourceName, level, navigationPrefix) {
        ...
        if (navigable && level < maxLevels) {
            ...
            Object.keys(type).filter(name => isIdentifier(name) && type[name].$Kind == 'NavigationProperty').forEach(name => {
                ...
                const restrictions = navigationPropertyRestrictions(root, navigationPath);
                if (restrictions.Navigability != 'None') {
    ...
    */
    ...
    function pathItemAction(paths, prefix, prefixParameters, actionName, overload, sourceName) {
        ...
        if (overload[voc.Core.LongDescription]) pathItem.post.description = overload[voc.Core.LongDescription];
