    ...
    function vocabularies(voc, alias) {
        ...
        Object.keys(terms).forEach(vocab => {
            ...
            terms[vocab].forEach(term => {
                const aliasKey = 'Org.OData.' + vocab + '.V1';
                const prefix = alias[aliasKey] || aliasKey;
                voc[vocab][term] = '@' + prefix + '.' + term;
            });
        });
    }
    ...
    function entityKey(entityType, level) {
        ...
        keys.forEach((key, index) => {
            ...
            if (typeof key == 'string') {
                ...
                const segments = Object.values(key)[0].split('/');
                parameter = segments.join('-')
