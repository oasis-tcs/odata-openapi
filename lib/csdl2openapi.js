...
module.exports.csdl2openapi = function (
    ...
    function vocabularies(csdl, alias) {
        const terms = {
            Capabilities: ['CountRestrictions', 'DeleteRestrictions', 'FilterRestrictions',
                'IndexableByKey', 'InsertRestrictions',
    ...
    function pathItems(paths, prefix, prefixParameters, element, root, sourceName, targetName, target, level) {
        ...
        operationRead(pathItem, element, name, sourceName, targetName, target, level);;
    ...
    function pathItemsWithKey(paths, prefix, prefixParameters, element, root, sourceName, targetName, target, level) {
        ...
        if (indexable) {
            ...
            operationRead(pathItem, element, name, sourceName, targetName, target, level, true);;
            operationUpdate(pathItem, element, name, sourceName, targetName, target, level);
            operationDelete(pathItem, name, sourceName, target, level);
    ...
    function operationCreate(pathItem, element, name, sourceName, targetName, target, level) {
        ...
        const targetInsertable = target == null
            ...
            || targetRestrictions.Insertable !== false;
     ...
     */
    function operationRead(pathItem, element, name, sourceName, targetName, target, level, byKey = false) {
        const targetRestrictions = target && target[voc.Capabilities.ReadRestrictions];
        const targetReadable = target == null
            || targetRestrictions == null
            || targetRestrictions.Readable !== false
            || (byKey == true && targetRestrictions.ReadByKeyRestrictions && targetRestrictions.ReadByKeyRestrictions.Readable !== false);

        if (targetReadable) {
            const summary = byKey || (level > 0 && !element.$Collection)
                ? targetRestrictions && targetRestrictions.ReadByKeyRestrictions && targetRestrictions.ReadByKeyRestrictions.Description
                : targetRestrictions && targetRestrictions.Description;
            const description = byKey || (level > 0 && !element.$Collection)
                ? targetRestrictions && targetRestrictions.ReadByKeyRestrictions && targetRestrictions.ReadByKeyRestrictions.LongDescription
                : targetRestrictions && targetRestrictions.LongDescription;
            ...
            if (collection) {
                ...
                optionOrderBy(operation.parameters, element, target);
            }
     ...
     * @param {string} target Target container child of path
     */
    function optionOrderBy(parameters, element, target) {
        const targetRestrictions = target && target[voc.Capabilities.SortRestrictions];
        const targetSortable = target == null
            || targetRestrictions == null
            || targetRestrictions.Sortable !== false;

        if (targetSortable) {
            const nonSortable = {};
            ((targetRestrictions && targetRestrictions.NonSortableProperties) || []).forEach(name => {
    ...
    function operationDelete(pathItem, name, sourceName, target, level) {
        ...
        const targetDeletable = target == null
            || targetRestrictions == null
            || targetRestrictions.Deletable !== false;
        if (targetDeletable) {
            const summary = targetRestrictions && targetRestrictions.Description;
            //TODO: description from LongDescription
            pathItem.delete = {
    ...
    function pathItemsWithNavigation(paths, prefix, prefixParameters, type, root, sourceName, level) {
        ...
        if (level < 5) {
            ...
            Object.keys(type).filter(name => isIdentifier(name) && type[name].$Kind == 'NavigationProperty').forEach(name => {
                ...
                const targetSetName = root.$NavigationPropertyBinding && root.$NavigationPropertyBinding[name];
