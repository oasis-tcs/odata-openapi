...
module.exports.csdl2openapi = function (
    ...
    const derivedTypes = {};
    preProcess(csdl, boundOverloads, derivedTypes);
     ...
     * @param {object} derivedTypes Map of type names derived types
     */
    function preProcess(csdl, boundOverloads, derivedTypes) {
            ...
            Object.keys(schema).filter(name => isIdentifier(name)).forEach(name => {
                ...
                if (Array.isArray(element)) {
                    ...
                    while (structuredType.$BaseType) {
                        if (!derivedTypes[structuredType.$BaseType]) derivedTypes[structuredType.$BaseType] = [];
                        derivedTypes[structuredType.$BaseType].push(qualifiedName);
                        structuredType = modelElement(structuredType.$BaseType);
                    }
                }
            });
        });
     ...
     */
    function info(csdl) {
        //TODO: Core.Description on schema of entity container
        ...
        return {
            title: entityContainer[voc.Core.Description] || 'Service for namespace ' + nameParts(csdl.$EntityContainer).qualifier,
    ...
    function pathItems(paths, prefix, prefixParameters, element, root, sourceName, targetName, target, level) {
        ...
        pathItem.get = operationRead(element, name, sourceName, targetName, level);;
    ...
    function pathItemsWithKey(paths, prefix, prefixParameters, element, root, sourceName, targetName, target, level) {
        ...
        pathItem.get = operationRead(element, name, sourceName, targetName, level, true);;
        pathItem.patch = operationUpdate(element, name, sourceName, targetName, level);
        operationDelete(pathItem, name, sourceName, target, level);
     ...
     */
    function operationRead(element, name, sourceName, targetName, level, byKey = false) {
        ...
        if (targetName && sourceName != targetName) operation.tags.push(targetName);
    ...
    */
    ...
    function schemas(csdl) {
        ...
        //TODO: find derived types per structured type
    ...
    function schemasForStructuredType(schemas, qualifier, name, type) {
        ...
        schemas[allName + '-update'] = {
            title: name + ' (for update)',
            type: 'object',
            properties: updProperties
        };

        //TODO: anyOf[ <derived types> ]
