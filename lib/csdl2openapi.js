...
module.exports.csdl2openapi = function (
  ...
  //TODO: use directly from model, ideally remove here
  //TODO: only used in pathItemsForBoundOperations()
  const boundOverloads = model.boundOverloads;
  //TODO: only used in schemasForStructuredType()
  const derivedTypes = model.derivedTypes;
  //TODO: only used in ref()
  const namespace = model.namespace;
  const namespaceUrl = model.namespaceUrl;
  ...
  const openapi = {
    ...
    tags: tags(model.entityContainer),
  ...
  function tags(container) {
    ...
    Object.keys(container ?? {})
      ...
      .forEach((child) => {
        ...
        const type = model.element(container[child].$Type);
        const description =
  ...
  function paths(container) {
    ...
    resources.forEach((name) => {
      ...
      if (child.$Type) {
        ...
        child = { ...child, $ContainsTarget: true };
        pathItems(
          paths,
          "/" + name,
          [],
  ...
  function primitivePaths(element, prefix = "") {
    ...
    const ignore = Object.entries(propsOfType)
      ...
      .filter((entry) => !model.element(entry[1].$Type));
  ...
  function optionSelect(parameters, element, target, restrictions) {
    ...
    if (selectSupport.Supported !== false) {
      ...
      Object.keys(properties)
        .filter(
          (key) =>
            properties[key].$Kind != "NavigationProperty" ||
            csdl.$Version === "2.0"
        )
  ...
  function navigationPathMap(type, map = {}, prefix = "", level = 0) {
    ...
    Object.keys(properties).forEach((key) => {
      if (properties[key].$Kind == "NavigationProperty") {
        ...
        navigationPathMap(
          model.element(properties[key].$Type),
          map,
          prefix + key + "/",
          level + 1
        );
  ...
  function key(entityType) {
    ...
    while (type) {
      ...
      type = model.element(type.$BaseType);
    }
    return keys;
  }
    ...
    parameters.forEach((p) => {
      ...
      if (
        ...
        param.description +=
          ...
          "of type " +
          namespaceQualifiedName(namespace, p.$Type || "Edm.String") +
   ...
   */
  function components(entityContainer) {
    const c = {
      schemas: schemas(entityContainer),
    };
   ...
   */
  function schemas(entityContainer) {
    const unordered = {};
  ...
  function propertiesOfStructuredType(type) {
    const properties =
      type && type.$BaseType
        ? propertiesOfStructuredType(model.element(type.$BaseType))
        : {};
    if (type) {
      Object.keys(type)
        .filter((name) => isIdentifier(name))
