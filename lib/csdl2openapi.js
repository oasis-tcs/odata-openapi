...
module.exports.csdl2openapi = function (
    ...
    function vocabularies(alias) {
        const terms = {
            ...
            Core: ['AcceptableMediaTypes', 'Computed', 'Description', 'Example', 'Immutable', 'LongDescription', 'OptionalParameter', 'Permissions', 'SchemaVersion'],
            JSON: ['Schema'],
    ...
    function optionExpand(parameters, element, target) {
        ...
        if (supported) {
            const expandItems = ['*'].concat(navigationPaths(element));
     ...
     */
    function navigationPaths(element, prefix = '') {
        const paths = [];
        const type = modelElement(element.$Type);
        ...
        Object.keys(properties).forEach(key => {
            if (properties[key].$Kind == 'NavigationProperty') {
                ...
                paths.push(...navigationPaths(properties[key], prefix + key + '/'));
            }
        })
        return paths;
    }
    ...
    function primitivePaths(element, prefix = '') {
        ...
        const type = modelElement(element.$Type);
        const properties = propertiesOfStructuredType(type);
        Object.keys(properties).filter(key => properties[key].$Kind != 'NavigationProperty').forEach(key => {
            if (!properties[key].$Type || nameParts(properties[key].$Type).qualifier == 'Edm') {
                paths.push(prefix + key)
            } else {
                const propertyType = modelElement(properties[key].$Type);
                if (propertyType && propertyType.$Kind == 'ComplexType') {
                    paths.push(...primitivePaths(properties[key], prefix + key + '/'));
                    // paths.push(prefix + key)
                } else if (propertyType) {
                    paths.push(prefix + key)
                } else {
                    console.warn('Unknown type: ' + properties[key].$Type);
                }
            }
        })
        return paths;
     ...
     */
    function navigationPathMap(type, map = {}, prefix = '') {
        const properties = propertiesOfStructuredType(type);
        Object.keys(properties).forEach(key => {
            if (properties[key].$Kind == 'NavigationProperty') {
                map[prefix + key] = properties[key];
            } else if (properties[key].$Type && !properties[key].$Collection) {
                navigationPathMap(modelElement(properties[key].$Type), map, prefix + key + '/');
            }
        })
        return map;
    }
    ...
    function schema(element, suffix = '', forParameter = false) {
        ...
        switch (element.$Type) {
            ...
            case 'Edm.Stream':
                ...
                if (jsonSchema) {
                    //TODO: ODATA-1275 only parse if annotation value is a string, use directly otherwise
                    //TODO: error handling
