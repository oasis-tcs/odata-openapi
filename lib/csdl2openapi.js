...
module.exports.csdl2openapi = function (
  ...
  function pathValuePrefix(typename) {
    ...
    //TODO: binary', geography', geometry'
    return `'`;
  }
      ...
      if (
        ...
        if (implicitAliases) {
          ...
          pathSegments.push(
            p.$Name +
              "=" +
              (p.$Nullable ? "" : pathValuePrefix(p.$Type)) +
              "{" +
              p.$Name +
              "}" +
              (p.$Nullable ? "" : pathValueSuffix(p.$Type))
          );
          param.in = "path";
        }
        param.name = p.$Name;
      ...
      if (property.$Kind == "NavigationProperty") {
        ...
        if (
          ...
          if (property.$ContainsTarget) {
            ...
            //HACK
            csdl.$$entityReferenceNeeded = true;
          }
        }
      } else {
        if (
  ...
  function schema(element, suffix = "", forParameter = false) {
    ...
    //TODO: if forParameter and $Nullable: use type:"string", add pattern that matches original type and allows null
    //TODO: here or better within the cases above?
    //TODO: pattern only for single-quoted values - string, binary, duration,
    //TODO: remove pathValuePrefix and pathValueSuffix also for key values
    // - Pro: solves "empty string" problem for keys
    // - Convenience: default value with single quotes?
    if (forParameter && element.$Nullable) {
      delete s.anyOf;
      delete s.example;
      if (s.format) s.format += ",null";
      s.type = "string";
      if (element.$Type?.startsWith("Edm.Int")) s.pattern = "^(null|-?\\d+)$";
      s.default = "null"; //TODO: @Core.OptionalParameter.DefaultValue
      //TODO: pattern corresponding to $Type, with single quotes for strings unless key-as-segment?
      if (!element.$Type || element.$Type === "Edm.String")
        s.pattern = `^(null|${pathValuePrefix(
          element.$Type
