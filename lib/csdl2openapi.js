...
module.exports.csdl2openapi = function (
  ...
  function pathItems(
    ...
    if (prefixParameters.length > 0)
      paths[prefix].parameters = prefixParameters;
        ...
        "application/json": {
          schema: type.$Collection
            ? {
                ...
                properties: {
                  [csdl.$Version > "4.0" ? "@count" : "@odata.count"]: ref(
                    "count"
                  ),
                  value: s,
                },
  ...
  function schemas(csdl) {
    ...
    for (const r of requiredSchemas.list) {
      ...
      switch (type.$Kind) {
        case "ComplexType":
        case "EntityType":
          schemasForStructuredType(s, r.namespace, r.name, type);
          break;
        case "EnumType":
          schemaForEnumerationType(s, r.namespace, r.name, type);
          break;
        case "TypeDefinition":
          schemaForTypeDefinition(s, r.namespace, r.name, type);
          break;
      }
    }
  ...
  function schemasForStructuredType(schemas, qualifier, name, type) {
    ...
    Object.keys(properties).forEach((name) => {
      ...
      if (property.$Kind == "NavigationProperty") {
        ...
        if (
          ...
          if (deepUpdate) updProperties[name] = schema(property, "-create");
        }
      } else {
        if (
          property[voc.Core.Permissions] == "Read" ||
    ...
    if (Object.keys(allProperties).length > 0)
      schemas[allName].properties = allProperties;

    schemas[creName] = {
      title: title + " (for create)",
      type: "object",
    };
    if (Object.keys(creProperties).length > 0)
      schemas[creName].properties = creProperties;
    ...
    if (Object.keys(updProperties).length > 0)
      schemas[updName].properties = updProperties;
  ...
  function ref(typename, suffix = "") {
    ...
    if (typename.indexOf(".") != -1) {
      ...
      if (url === "" && !requiredSchemas.used[name]) {
        requiredSchemas.used[name] = true;
        requiredSchemas.list.push({ namespace: nsp, name: parts.name });
      }
