 ...
 * @param {Array} externalCsdl Optional array of CSDL documents
...
module.exports.csdl2openapi = function (
    ...
    const namespaceUrl = {};
    preProcess(csdl, boundOverloads, derivedTypes, alias, namespace, namespaceUrl, externalCsdl);
     ...
     * @param {Array} externalCsdl Optional array of CSDL documents
     */
    function preProcess(csdl, boundOverloads, derivedTypes, alias, namespace, namespaceUrl, externalCsdl) {
        inlineExternalCsdl(csdl, externalCsdl);
        ...
        Object.keys(csdl.$Reference || {}).forEach(url => {
            ...
            (reference.$Include || []).forEach(include => {
                ...
                if (!csdl.hasOwnProperty(include.$Namespace)) {
    ...
    /**
     * Inline schemas from external CDSL documents into main document
     * @param {object} csdl Main CSDL document
     * @param {Array} externalCsdl External CSDL documents
     */
    function inlineExternalCsdl(csdl, externalCsdl) {
        externalCsdl.forEach(ext => {
            //TODO: parse references in order to normalize qualifiers
            Object.keys(ext).filter(name => isIdentifier(name)).forEach(name => {
                csdl[name] = ext[name];
                //TODO: normalize all qualifiers to namespaces
                // - $Type
                // - $UnderlyingType
                // - $BaseType
                // - annotation targets
    ...
    function vocabularies(alias) {
        const terms = {
            ...
            Core: ['AcceptableMediaTypes', 'Computed', 'Description', 'Example', 'Immutable', 'LongDescription', 'Permissions', 'SchemaVersion'],
            JSON: ['Schema'],
    ...
    function resourceDiagram(csdl) {
                ...
                .forEach(typeName => {
                    ...
                    Object.keys(type).filter(name => isIdentifier(name)).forEach(propertyName => {
                        ...
                        if (property.$Kind == 'NavigationProperty' || targetNP.qualifier != 'Edm') {
                            ...
                            const bidirectional = property.$Partner && target[property.$Partner].$Partner == propertyName;
        ...
        Object.keys(entityContainer).filter(name => isIdentifier(name)).forEach(name => {
            ...
            if (resource.$Type) {
                ...
                if (resource.$Action) {
                    ...
                    const overloads = modelElement(resource.$Function).filter(overload => !overload.$IsBound);
    ...
    function pathItems(paths, prefix, prefixParameters, element, root, sourceName, targetName, target, level, navigationPath) {
        ...
        if (element.$ContainsTarget) {
            if (element.$Collection) {
                if (level < maxLevels)
                    pathItemsWithKey(paths, prefix, prefixParameters, element, root, sourceName, targetName, target, level, navigationPath, restrictions);
            } else {
                operationUpdate(pathItem, element, name, sourceName, targetName, target, level, restrictions);
                if (element.$Nullable) {
     ...
     */
    function pathItemsWithKey(paths, prefix, prefixParameters, element, root, sourceName, targetName, target, level, navigationPath, restrictions) {
    ...
    function optionOrderBy(parameters, element, target, restrictions) {
        ...
        if (sortRestrictions.Sortable !== false) {
            ...
            const type = modelElement(element.$Type) || {};
            const orderbyItems = [];
            Object.keys(type).filter(key => isIdentifier(key) && type[key].$Kind != 'NavigationProperty').forEach(key => {
                if (!nonSortable[key]) {
     ...
     * @param {object} restrictions Navigation property restrictions of navigation segment
     */
    function operationUpdate(pathItem, element, name, sourceName, targetName, target, level, restrictions) {
    ...
    */
    ...
    function pathItemAction(paths, prefix, prefixParameters, actionName, overload, sourceName, actionImport = {}) {
        const pathItem = {
            post: {
                summary: actionImport[voc.Core.Description] || overload[voc.Core.Description] || 'Invoke action ' + nameParts(actionName).name,
    ...
    function pathItemFunctionImport(paths, name, child) {
        ...
        overloads.filter(overload => !overload.$IsBound).forEach(overload => pathItemFunction(paths, '/' + name, [], child.$Function, overload, child.$EntitySet, child));
    }
    ...
    function pathItemFunction(paths, prefix, prefixParameters, functionName, overload, sourceName, functionImport = {}) {
        ...
        parameters.forEach(p => {
            ...
            if (p.$Collection || p.$Type == 'Edm.Stream'
                ...
                // TODO: path value prefix/suffix also needed for key-as-segment
        ...
        const pathItem = {
            get: {
                summary: functionImport[voc.Core.Description] || overload[voc.Core.Description] || 'Invoke function ' + nameParts(functionName).name,
                tags: [sourceName || 'Service Operations'],
    ...
    function schemas(csdl) {
        ...
        if (csdl.$EntityContainer) s.error = error();
        ...
        authorizations.forEach(auth => {
            ...
            switch (type) {
                ...
                case 'OpenIDConnect':
                    ...
                    break;
                default:
                    console.assert(false, 'Unknown Authorization type ' + auth['@type']);
            }
            schemes[auth.Name] = scheme;
        });
        if (authorizations.length > 0) components.securitySchemes = schemes;
    }
