...
class EDM {
  //TODO: stronger encapsulation, make private?
  elements = {};
  ...
  addDocument(csdl, messages) {
    this.preProcess(csdl, messages);
    //TODO: only for first? document
    this.entityContainer = this.elements[csdl.$EntityContainer];
  }

  /**
   * Extract model data from CSDL document
   * @param {object} csdl CSDL document
   * @param {Array} messages Warnings
   */
  preProcess(csdl, messages) {
  ...
  element(qualifiedName) {
    const q = nameParts(qualifiedName);
    //TODO: use namespaceQualifiedName()
    return this.elements[`${this.namespace[q.qualifier]}.${q.name}`];
  }
  ...
  //TODO: combine with identically named function in csdl2openapi.js
