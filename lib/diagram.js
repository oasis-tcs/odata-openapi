 ...
 */
function resourceDiagram(model, rootResourcesToKeep, entityTypesToKeep) {
  let diagram = "";
  let comma = "";
  ...
  // entityTypesToKeep contains alias-qualified names, model.structuredTypes contains namespace-qualified names
  const aliasQualifiedTypesToKeep = entityTypesToKeep
    ? entityTypesToKeep.map((name) => model.namespaceQualifiedName(name))
    : null;

  for (const [qualifiedName, type] of model.structuredTypes) {
    if (
      aliasQualifiedTypesToKeep &&
      !aliasQualifiedTypesToKeep.includes(qualifiedName)
    )
      continue;
