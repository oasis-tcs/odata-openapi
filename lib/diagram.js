...
function resourceDiagram(model) {
  ...
  //TODO: for(const schema of model.schemas) // hide access details in model
  Object.keys(model.csdl)
    .filter((name) => isIdentifier(name))
    .forEach((namespace) => {
      const schema = model.csdl[namespace];
      Object.keys(schema)
        .filter(
          (name) =>
            isIdentifier(name) &&
  ...
  Object.keys(model.entityContainer)
    .filter((name) => isIdentifier(name))
    .reverse()
    .forEach((name) => {
      const resource = model.entityContainer[name];
      if (resource.$Type) {
        diagram +=
          comma +
          "[" +
